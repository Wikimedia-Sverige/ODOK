...
#   View -tmp2.txt at the relevant wikipedia to manually fix any
#   disambiguations and to verify that the articles are about the right person
...
#       i.e. groupName [memberA;memberB];singleA -> memberA;memberB
#            (should give memberA;memberB:singleA)
#
import codecs
import common
...
def makeSource(source):
    ...
    d = {'id': None, 'name': None, 'wiki': None,
         'real_id': None, 'url': None, 'cmt': None}
    for l in lines:
...
def fileWithWikidata(filename=u'Artists.csv', idcol=0, namecol=[4, 3], verbose=False):
    '''
    Given a csv file containing object ids and artist names this checks
        for matching pages on a wikipedia.
    This can either be a csv file with one line per item or with one line
        per artist and idnos semicolonseparated
    If the page exists but does not contain a wikidata_item then one is created.
    It returns two files:
    *-tmp.txt  which contains artistnames with mapped wikidata items and
        any comments
    *-tmp2.txt which contains wikicode of the above
    ...
    if '' in aDict.keys():
        # remove any artistless entries (these cause issues with wikidata)
        del aDict['']
...
def artistFromLog(filename='artist-dump.csv', idcol=0, namecol=3, verbose=False):
    ...
    makeCSV(aDict, '%s-tmp.txt' % outfile)
    makeWiki(aDict, '%s-tmp2.txt' % outfile)
    # Check file as normal + for name formating then
    # openCSV(u'%s-tmp.txt' % outfile, prefix=u'')
...
def wikidataFromOdok(wdList, aDict, dbReadSQL):
    ...
    # resonse is a list of odok_ids with wd info, change to become a
    # list of wd_ids with odok_info
...
def addArtist(fName, lName, wikidata, ids):
    ...
    query = u'INSERT INTO artist_table (first_name, last_name, wiki) VALUES '
    query += u'("%s","%s","%s");\n' % (fName, lName, wikidata)
    query += u'SET @last_artist_id = LAST_INSERT_ID();\n'
