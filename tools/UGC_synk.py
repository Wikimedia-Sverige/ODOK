...
def run(checkList, wpApi, dbWriteSQL):
    changed=0
    log=''
    checkPages = wpApi.getPage(checkList)
    for pagename, contents  in checkPages.iteritems():
        changes = UGConPage(pagename, contents, dbWriteSQL)
        if changes:
    ...
    for l in lines:
        if inTemplate:
            if l.strip().startswith(u'|}'):
                ...
                changed = changed+1
                newId = getNewId(pagename, dbWriteSQL)
...
def getNewId(pagename, odokWriter):
    #This should ideally be worked into odok.py and be made SQL safe
    queries = []
    queries.append(u'INSERT INTO UGC_table (listpage) VALUES ("%s"); ' %pagename)
