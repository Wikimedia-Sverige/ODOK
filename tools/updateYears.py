...
# -*- coding: utf-8  -*-
'''
Tool for scraping artistYears from sv.wiki articles and updating these
...
def getArtists(dbWriteSQL):
    ...
    :returns list of artists
             each a dict {id, wikidata, birth_year, death_year, svwiki}
    '''
    ...
    for row in results:
        ...
        artists.append({u'id': idNo,
                        u'wikidata': wikidata,
                        u'birth_year': birth_year,
                        u'death_year': death_year,
                        u'svwiki': None})
    return artists
...
def getArticles(wdApi, artists, verbose=False):
    '''
    Queries Wikidata's API to find the articles corresponding to each
    wikidata entry
...
def getYears(wpApi, artists, verbose=False):
    '''
    Identifies any artists with updated years by looking at the relevant
    categoires at the sv.Wikipedia page for said artist
    :param artists: list of artists
                    each a dict {id, wikidata, birth_year, death_year, svwiki}
    :returns list of artists
             each a dict {id, wikidata, birth_year, death_year, svwiki}
    ...
    catDict = wpApi.getPageCategories(withSvWiki.values(),
                                      nohidden=True,
                                      dDict=None,
                                      debug=verbose)
    ...
    for a in artists:
        ...
        if idNo in withSvWiki.keys():
            if not withSvWiki[idNo] in catDict.keys():
                ...
                continue
            (birth, death) = getYearCats(catDict[withSvWiki[idNo]],
                                         withSvWiki[idNo])
...
def uppdateArtist(dbWriteSQL, artists):
    '''
    ...
    for a in artists:
        ...
        if a[u'death_year']:
            if a[u'birth_year']:
                query += u""", `death_year`=%r"""
            else:
                query += u"""`death_year`=%r"""
            vals.append(a[u'death_year'])
        query += u""" WHERE `id` = %s;\n"""
        vals.append(a[u'id'])
        dbWriteSQL.query(query[:-1], tuple(vals), expectReply=False)
...
def run(testing=False):
    ...
    for c in changedArtists:
        print '%d\t%r\t%r\t%s' % (c[u'id'], c[u'birth_year'],
                                  c[u'death_year'], c[u'svwiki']['title'])
    if len(changedArtists) > 0:
        uppdateArtist(dbWriteSQL, changedArtists)
